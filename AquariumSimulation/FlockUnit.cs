...
public class FlockUnit : MonoBehaviour
{
	...
	private void goForFood(int index)
	{
		var foodPosition = assignedFlock.foodPositions[index];
		var moveVector = foodPosition;
		moveVector -= myTransform.position;
		moveVector = moveVector.normalized * speed;
	...
	private int seekForFood()
	{
		float closest = 32000;
		int index = -1;
		for (int i = 0; i < assignedFlock.foodPositions.Length; i++)
		{
			if (IsInFOV(assignedFlock.foodPositions[i]))
			{
				float distanceToFood = Vector3.Distance(assignedFlock.foodPositions[i], myTransform.position);
				if (distanceToFood < assignedFlock.foodDistance)
				{
					if (distanceToFood < closest)
					{
						if (distanceToFood < 0.85)
						{
							assignedFlock.EatFood(i);
							return -1;
						}
						closest = distanceToFood;
						index = i;
					}
				}
			}
